...
import { listClients, listStaff, type ClientFilters } from "@/lib/api/admin";

/**
 * Client list.
 *
 * Section 6.2 requires the assigned manager to be visible alongside each user,
 * so it is a column rather than something you open a record to discover.
 *
 * A Manager sees only their own clients here — enforced by the API, not by this
 * page, so the filters below narrow a list that is already scoped.
 */
export default function ClientsPage() {
  ...
  return (
    <div>
      <PageHeader
        title="Clients"
        description="Every client account, with its assigned manager and status."
        actions={
          <Link
            href="/admin/invites"
      ...
      >
        <Input
          ...
          placeholder="Search by company, owner, reference or email"
          value={search}
